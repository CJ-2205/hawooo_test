...
public partial class user_static_72HhotDeal : System.Web.UI.Page
{
    ...
    {
        string sqlTxt =
            "SELECT SPM01,SPM04,SPM05 FROM SPRODUCTSM WHERE SPM01 IN (686)";
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = sqlTxt;
        DataTable dt = SqlDbmanager.queryBySql(cmd);
        DateTime etime = Convert.ToDateTime(dt.Rows[0]["SPM05"].ToString());
        if (DateTime.Now >= Convert.ToDateTime(etime))
        {
            Response.Redirect("../user/72HhotDeal2.aspx");
        }
    }
    ...
    {
        string sqlTxt =
            "SELECT SPM01,SPM04,SPM05 FROM SPRODUCTSM WHERE SPM01 IN (486) ";
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = sqlTxt;
    ...
    //        {
    //            {0, ConfigurationManager.AppSettings["imgUrl"]+"ftp/week1/pd_01.png"},
    //            {1, ConfigurationManager.AppSettings["imgUrl"]+"ftp/week1/pd_02.png"},
    //            {2, ConfigurationManager.AppSettings["imgUrl"]+"ftp/week1/pd_03.png"},
    //            {3, ConfigurationManager.AppSettings["imgUrl"]+"ftp/week1/pd_04.png"}
    //        };
    //        return list;
    //    }
    //}
