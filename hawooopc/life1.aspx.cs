...
public partial class user_life1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["desktop"] == null)
            {
                if (ismobile)
                {
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "msg", "location.href='../mobile/life1.aspx'", true);
                }
            }
    ...
    private void bindDT()
    {
        ...
        searchProp.JoinTxts.Add("INNER JOIN (SELECT SPD01 AS SPD01,SPD02 AS SPD02 FROM SPRODUCTSD WHERE SPD01 IN ('528','540','541','542') ) AS DT ON  WP01=DT.SPD02  ");
        ...
        if(dt.Rows.Count > 0)
{
   if (dt.Select("SPD01='528' ").Length > 0)
        {
            rp0.DataSource = dt.Select("SPD01='528'").CopyToDataTable();
            rp0.DataBind();
        }
