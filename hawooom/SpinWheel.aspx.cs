...
public partial class mobile_static_SpinWheel : System.Web.UI.Page
{
    ...
    public void GetUserInfo(int UID)
    {
        ...
        _playTimes = _swRewardFac.GetPrizeLogDt(UID, DateTime.Now).Rows.Count;
    ...
    public void RenderPrize(int userid, string lg)
    {
        ...
        if (HaCoin >= 100)
        {
            ...
            MTrialCard mtCard = new MTrialCard(userid);
            ...
            //filter.userRewardType = EmUserRewardType.Test.ToString();
            filter.isGetBlackCard = (!_isRealBlack && !mtCard.CheckTrialBlack()) ? true : false;
            filter.isGetThousandCoin = _swRewardFac.GetPrizeLogDt(userid, DateTime.Now, EmReward.ThousandHaCoin.GetHashCode()).Rows.Count.Equals(0) ? true : false;
