...
public partial class user_hotsale_detail : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["desktop"] == null)
            {
                if (ismobile)
                {
                    ScriptManager.RegisterStartupScript(Page, typeof(Page), "msg", "location.href='../mobile/hotsale_detail.aspx?" + Request.QueryString + "';", true);
                }
            }
    ...
    private void bindDT(int id)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            string strSql = "SELECT SPI01,SPI04,SPI05 FROM SPRODUCTSI WHERE SPI02='H01' AND SPI01=@SPI01";
            cmd = new SqlCommand();
            cmd.CommandText = strSql;
            cmd.Parameters.Add(SafeSQL.CreateInputParam("SPI01", SqlDbType.Int, id));
            DataTable imgDT = SqlDbmanager.queryBySql(cmd);
            if (imgDT.Rows.Count > 0)
            {
                lit_img.Text = "<img src='../images/adimgs/" + imgDT.Rows[0]["SPI04"].ToString() + "' style='width:100%'></img>";
            }
            if (_sType == 1)
            {
