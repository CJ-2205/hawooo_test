...
public partial class user_productdetail2 : System.Web.UI.Page
{
    ...
    private void BindBrand()
    {
        if (_mainDt.Rows.Count > 0)
        {
            ...
            hf_SupName.Value = _mainDt.Rows[0]["B02"].ToString();
            hf_brand.Value = _mainDt.Rows[0]["BA02"].ToString();
            if (!_mainDt.Rows[0]["BA02"].ToString().Equals(""))
                lit_B01.Text = "<h5>Brand：<a href='" + BrandEventBL.GetBrandLink(_mainDt.Rows[0]["B01"].ToString(), BrowserType.PC) + "'>" + _mainDt.Rows[0]["BA02"] +
                               "</a></h5>";
            else
                lit_B01.Text = "";
        }
    ...
    public void BindComment(string optionId)
    {
        ScriptManager.RegisterStartupScript(Page, typeof(Page), "comment", "GetHwComment('" + optionId + "');GetHwCommentRight('" + optionId + "');", true);
    ...
    public enum StockType { OutOfStock, InStock };

    private void CreateMicroData()
    {
        if (_mainDt.Rows.Count > 0)
        {
            ProductSchemaHelper helper = new ProductSchemaHelper();

            List<SchemaProduct> products = new List<SchemaProduct>();

            var schemaString = helper.GenerateProductSchemaHtml(products);
            LiteralControl schemaHtml = new LiteralControl(schemaString);
            placeholder_product.Controls.Add(schemaHtml);

            bool status = true;
            string WP07 = _mainDt.Rows[0]["WP07"].ToString();
            DateTime WP09 = Convert.ToDateTime(_mainDt.Rows[0]["WP09"].ToString());
            DateTime WP10 = Convert.ToDateTime(_mainDt.Rows[0]["WP10"].ToString());
            if (!WP07.Equals("1"))
            {
                status = false;
            }
            if (DateTime.Now < WP09 && DateTime.Now > WP10)
            {
                status = false;
            }
            string isBookingState = _mainDt.Rows[0]["WP28"].ToString();
            if (isBookingState != "1")
            {
                var selStock = _detailDt.AsEnumerable().Where(v => v.Field<int>("SCOUNT") > 0).Select(v => v);
                if (!selStock.Any())
                {
                    status = false;
                }
            ...
            products.Add(new SchemaProduct()
            {
                ProductId = _mainDt.Rows[0]["WP01"].ToString(),
                Brand = _mainDt.Rows[0]["B02"].ToString(),//品牌名稱
                Name = _mainDt.Rows[0]["WP02"].ToString(),//商品名稱
                Url = "https://www.hawooo.com/user/productdetail.aspx?id=" + _mainDt.Rows[0]["WP01"].ToString(),//商品URL
                Availability = status ? StockType.InStock : StockType.OutOfStock,//StockType
                Condition = "NewCondition",//
                Description = _mainDt.Rows[0]["WP02"].ToString(),//商品名稱
                Image = "https://p8r7m4d5.stackpathcdn.com/images//webimgs/" + _mainDt.Rows[0]["WP08_1"].ToString(),//主圖SRC
                Price = PbClass.CashRate(_detailDt.Rows[0]["WPA06"].ToString(), Application["mycashrate"].ToString()),//價格，每個規格價格不一樣
                PriceCurrency = "MYR",
            });
    ...
    private void BindDt(int pId)
    {
        ...
        if (_mainDt.Rows.Count > 0)
        {
            ...
            //FB Product Micro Data: Howard
            BindProductMicroData();
