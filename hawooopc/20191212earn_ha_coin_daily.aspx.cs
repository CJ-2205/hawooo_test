...
public partial class user_static_20191212earn_ha_coin_daily : System.Web.UI.Page
{
    ...
    private static int MemberGetCoin(string memberID)
    {
        ...
        if (Session["A01"] != null)
        {
            foreach (RepeaterItem item in rp_date.Items)
            {
                ...
                if (i <= _signCount)//判別簽過幾次(打開幾張卡片)
                {
                    ...
                    li.Attributes["onclick"] = "alert('Only check in once a day!')";
                }
            }
            ...
            if (dt.Rows.Count < _signLimit)//今天尚未簽到跑這邊
            {
                ...
                {
                    var li = rp_date.Items[i - 1].FindControl("li1") as HtmlGenericControl;
                    li.Attributes["onclick"] = "alert('You cannot check in')";
                }
            }
            else//已經簽到跑這邊
            {
    ...
    public static string GetCoin(string userID)
    {
        ...
        {
            returnMsg = "Only check in once a day!";
        }
