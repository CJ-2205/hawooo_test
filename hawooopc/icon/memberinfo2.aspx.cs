...
public partial class user_memberinfo : System.Web.UI.Page
{
    ...
    private void GetData(int A01)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            if (dt.Rows[0]["A23"].ToString().Equals("1"))
            {
                ...
                btnSend.InnerText = "已認證";
            }
            else
            {
                btnSend.InnerText = "點我認證";
            }
    ...
    //})
    //";
    //        ScriptManager.RegisterStartupScript(Page,GetType(),"block",str,true);
    //    }
    ...
    //    objA.A08 = txt_A08.Text;
    //    //CFacade.GetFac.GetAFac.SendJoinEdmMsg(objA);
    //    string str = @"setTimeout($('.btn-block').removeAttr('disabled'), 30000);";
    //    //string str = @"setTimeout(alert('HI'), 30000);";
    //    ScriptManager.RegisterStartupScript(Page, GetType(), "block", str, true);
    //    ScriptManager.RegisterStartupScript(Page, GetType(), "alert", "openModal('member1')", true);
    //}
}