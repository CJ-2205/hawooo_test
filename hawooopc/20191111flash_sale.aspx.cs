...
public partial class user_static_20191111flash_sale : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ismobile)
            {
                Response.Redirect("../mobile/20191111flash_sale.aspx" + Request.Url.Query);
            }
            BindGoods();
        }
    }
    ...
    private void BindGoods()
    {
        if (_eventId != 0)
        {
            ...
            {
                rp.DataSource = bindDt.Select("SPD08='A'").CopyToDataTable();
                rp.DataBind();
            }
            if (bindDt.Select("SPD08='B'").Length > 0)
            {
                rp2.DataSource = bindDt.Select("SPD08='B'").CopyToDataTable();
                rp2.DataBind();
            }
            if (bindDt.Select("SPD08='C'").Length > 0)
            {
                rp3.DataSource = bindDt.Select("SPD08='C'").CopyToDataTable();
                rp3.DataBind();
            }
            if (bindDt.Select("SPD08='D'").Length > 0)
            {
                rp4.DataSource = bindDt.Select("SPD08='D'").CopyToDataTable();
                rp4.DataBind();
            }
    ...
    /// 
    /// </summary>
    /// <param name="sold">SPD07安计秖</param>
    /// <param name="stock">SPD06计秖</param>
    /// <returns></returns>
    ...
    private DataTable TransDt(DataTable sdt)
    {
        ...
        dt.Columns.Add("SPD05"); //舦
        dt.Columns.Add("SPD06"); //计秖,畐
        dt.Columns.Add("SPD07"); //安计秖
