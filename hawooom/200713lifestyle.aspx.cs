...
public partial class mobile_static_200713lifestyle : System.Web.UI.Page
{
    private int HiglightedEventId = 1054; //1054
    public string cacheVersion = "1";
    private string _stime = "2020-07-13 12:00:00";
    ...
    private DataTable _productHb = new DataTable();
    private void BindHb()
    {
        if (HiglightedEventId != 0)
        {
            ...
            _productHb = TransDt(dt);

            if (_productHb.Rows.Count >= 8)

            {
                hb_rp.DataSource = _productHb;
                hb_rp.DataBind();
            }
    ...
    protected void hb_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            int pid = Convert.ToInt32(((HiddenField)e.Item.FindControl("hfWP01")).Value);
            var options = _productHb.AsEnumerable().Where(v => v.Field<Int64>("WP01").Equals(pid))
