...
public partial class mobile_memberborder : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ((Literal)member_class.FindControl("lit_class_txt")).Text = "代購單列表";
            if (Session["A01"] != null)
            {
                bindDT();
            }
            ...
            {
                Response.Redirect("login.aspx?rurl=memberborder.aspx");
            }
        }
    }
    private void bindDT()
