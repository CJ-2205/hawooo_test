...
public partial class user_member_card : System.Web.UI.Page
{
    ...
    private void BindCardInfo()
    {
        ...
        if (Session["A01"] != null && Session["IsBlackExp"] != null)
        {
            ...
            if (MDT.Rows.Count > 0)
            {
                ...
                //getCardTime = Convert.ToDateTime(MDT.Rows[0]["MCTime"]).ToString("yyyy-MM-dd HH:mm:ss");
                litMsg.Text = @"了解更多會員制度及優惠。";
                if (IsBlackExp.Equals("YES"))
                {
                    cardType = "B";
                    //getCardTime = DateTime.Now.ToString("yyyy-MM-01 00:00:00");
                    ...
                    TrialBlackCard.Visible = true;
                    Point.Visible = false;
                    TrailBefore.Visible = false;
                }
    ...
    {
        _sb.Append("<li style='padding-left:37px'>");
