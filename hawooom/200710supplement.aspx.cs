...
public partial class mobile_static_200710supplement: System.Web.UI.Page
{
    public string cacheVersion = "2";

    private int Buy1Free1EventId = 1043; //1043
    private int TopSellersEventId = 1044;   //1044

    private string _stime = "2020-06-28 12:00:00";

    private int Buy1Free1Products = 6;
    private int TopSellersProducts = 6;
    ...
    private void BindBuy1Free1()
    {
        if (Buy1Free1EventId != 0)
        {
            ...
            if (_productDtBuy1Free1.Rows.Count >= 8)

            {
                rp_buy1free1.DataSource = _productDtBuy1Free1.AsEnumerable().Take(this.Buy1Free1Products).CopyToDataTable();
    ...
    private DataTable _productDtTopSellers= new DataTable();
    private void BindTopSellers()
    {
        if (TopSellersEventId != 0)
        {
            SearchProp prop = new SearchProp();
            prop.Cells.Add("SPD01");
            prop.Cells.Add("SPD07");
            ...
            //prop.Cells.Add("WPA11");
            //prop.JoinTxts.Add("INNER JOIN SPRODUCTSD ON SPD02=WP.WP01 AND SPD01=@SPD01 ");
            prop.SelectIDS.Add(TopSellersEventId);
            ...
            DataTable dtRealStock = GetRealStock(TopSellersEventId, _stime);
            ...
            _productDtTopSellers = TransDt(dt);

            if (_productDtTopSellers.Rows.Count >= 1)

            {
                rp_topseller.DataSource = _productDtTopSellers.AsEnumerable().Take(this.TopSellersProducts).CopyToDataTable();
                rp_topseller.DataBind();
            }
    ...
    protected void rp_topseller_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            int pid = Convert.ToInt32(((HiddenField)e.Item.FindControl("hfWP01")).Value);
            var options = _productDtTopSellers.AsEnumerable().Where(v => v.Field<Int64>("WP01").Equals(pid))
                .OrderByDescending(v => v.Field<int>("SPD05"));
