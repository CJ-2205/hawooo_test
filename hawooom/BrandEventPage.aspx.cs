    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                {
                    Response.Redirect("https://www.hawooo.com/mobile/brand_1.aspx?bid=" + Request.QueryString["bid"].ToString());
                }
            }
        }
    }
...
WHERE MSysId=@MSysId ORDER BY BlockId";
    ...
    private void BindEventPage()
    {
        ...
        string Content = _dtMSrc.Rows[0]["MBlockInfo"].ToString();
    ...
    private List<GoodsTemplate> CreateGoodTemplateList()//回傳各選品的區塊樣版
    {
        ...
        {
            dtGoodsTempInfo = dt.Select("STempName='" + dr["BlockStyle"].ToString() + "' and STempType=2").CopyToDataTable();//取得商品區樣版的DataRow
