...
public partial class user_fastlist : System.Web.UI.Page
{
    ...
    {
        DataTable dt = CFacade.UserFac.GetShopIndexList("F");
        ...
        DataSet ds = CFacade.UserFac.GetShopEventProducts(EIDLIST);
        int i = 0;
        foreach (RepeaterItem ri in rp_event_list.Items)
        {
            ...
            if (D01.Length > 0)
            {
                ((Literal)ri.FindControl("lit_D01")).Text = "<a href=\"" + D01[0]["SPI05"].ToString() + "\"><img src=\"../images/adimgs/" + D01[0]["SPI04"].ToString() + "\" style=\"max-width:960px\" /></a>";
            }
            ...
            ((Repeater)ri.FindControl("rp_product_list")).DataSource = ds.Tables[i];
