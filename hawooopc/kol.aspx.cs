...
public partial class user_kol : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["desktop"] == null)
            {
                if (ismobile)
                {
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "msg", "location.href='../mobile/kol.aspx'", true);
                }
            }
    ...
    private void bindDT()
    {
        ...
        searchProp.JoinTxts.Add("INNER JOIN (SELECT SPD01 AS SPD01,SPD02 AS SPD02 FROM SPRODUCTSD WHERE SPD01 IN ('552','553','554','555') ) AS DT ON  WP01=DT.SPD02  ");
        ...
        if (dt.Select("SPD01='552' ").Length > 0)
        {
            rp1.DataSource = dt.Select("SPD01='552'").CopyToDataTable();
            rp1.DataBind();
        }
        if (dt.Select("SPD01='553' ").Length > 0)
        {
            rp2.DataSource = dt.Select("SPD01='553'").CopyToDataTable();
            rp2.DataBind();
        }
        if (dt.Select("SPD01='554' ").Length > 0)
        {
            rp3.DataSource = dt.Select("SPD01='554'").CopyToDataTable();
            rp3.DataBind();
        }
        if (dt.Select("SPD01='555' ").Length > 0)
        {
            rp4.DataSource = dt.Select("SPD01='555'").CopyToDataTable();
            rp4.DataBind();
        }
