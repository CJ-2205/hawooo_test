...
public partial class WorldCupSpin : System.Web.UI.Page
{
    //世足版輪盤
    ...
    DateTime eventEnd = new DateTime(2018, 07, 18, 23, 59, 59);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (today > eventEnd)
        {
            Response.Redirect("../user/spinwheel.aspx");
        }
        else
        {
            ...
            if (isMobile)
            {
                ...
                Response.Redirect("../mobile/WorldCupSpin.aspx" + qstr);
            }
            else
            {
                if (Session["A01"] != null)
...
WHERE PLOG03=@ORM23 AND PLOG07='180620WorldCupSpin'";        //是否轉過

        string sqlOrm = @"SELECT TOP 3 ORM01,ORM03,ORM08 FROM ORDERM
WHERE ORM23=@ORM23 AND  ORM03 BETWEEN '2018-04-01 00:00:00' AND '2018-07-14 23:59:59' AND ORM19=1 AND ORM24>0
ORDER BY ORM03 DESC";        //訂單明細          
...
END
 FROM PRIZELOG
WHERE Plog03=@A01 AND PLOG07='180620WorldCupSpin' ORDER BY PLOG02 DESC";          //User玩過的Log
...
PLOG07='180620WorldCupSpin' AND PLOG03=@PLOG03
 AND (DAY(PLOG02)=DAY(GETDATE()) AND MONTH(PLOG02)=MONTH(GETDATE())  AND YEAR(PLOG02)=YEAR(GETDATE()))";
        ...
        if (dtLog.Rows.Count < spinLimit)           //不能超過當天上限
        {
            ...
            cmd.Parameters.Add(SafeSQL.CreateInputParam("@PLOG07", SqlDbType.VarChar, "180620WorldCupSpin"));
            ...
            {
                statusDeduct = coinFac.DeductCoin(userid, 100, "WorldCupCoin").ToUpper();
    ...
    public static string AddHaCoin(int userID, int pID)
    {
        ...
        string i = coinFac.AddCoin(userID, rebate, "WorldCupCoin");
        return i.ToUpper();
    }
