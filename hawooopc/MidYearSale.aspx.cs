...
public partial class user_static_MidYearSale : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ismobile)
                Response.Redirect("../mobile/MidYearSale.aspx");

            DataTable dt = BindData(725);
            var rand = new Random();
            var employees = dt.AsEnumerable().OrderBy(r => rand.Next()).Take(10);
            //var products = dt.AsEnumerable().OrderBy(x => rand.Next()).Take(8).CopyToDataTable();
