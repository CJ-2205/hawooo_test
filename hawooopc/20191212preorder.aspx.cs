...
public partial class user_20191212preorder : System.Web.UI.Page
{
    //private int eid = 839;
    //private int eid = 817;//代刚盡ノ
    //private int eid = 798;//代刚盡ノ
    private int eid = 777;//代刚盡ノ

    protected void Page_PreLoad(object sender, EventArgs e)
    {
        if (DateTime.Now >= Convert.ToDateTime("2020-12-12 00:00:00"))
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("../mobile/" + "20191212preorder.aspx");
            }
            BindProductList(eid);
            BindAddList();
        }
    ...
    protected void rpPreProducts_OnItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            decimal Discount = options.Min(p => p.Field<decimal>("WPA06"))- options.Min(p => p.Field<decimal>("WPA10"));//12/4穝糤竕﹚чΙ基
