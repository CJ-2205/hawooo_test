...
public partial class user_static_200710supplement : System.Web.UI.Page
{
    public int serverType = 1;
    public string cacheVersion = "3";
    public Dictionary<string, dynamic> optionBuy1free1 = new Dictionary<string, dynamic>();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (ismobile)
            {
                Response.Redirect("../mobile/200710supplement.aspx" + Request.Url.Query);
            }

            // Related to "special stylee goods", but have no "Fire and pregress bar" logic:
            setOptions();

            BindSpecialStyleeGoods(optionBuy1free1);
    ...
    {
        optionBuy1free1.Add("webControlIds", rpBuy1free1);
        optionBuy1free1.Add("eventId", toggleSvr<int>(serverType, 1043, 798));
        optionBuy1free1.Add("take", toggleSvr<int>(serverType, 8, 7));
        optionBuy1free1.Add("productDt", new DataTable());

        optionTopSellers.Add("webControlIds", rpTopSellers);
        optionTopSellers.Add("eventId", toggleSvr<int>(serverType, 1044, 798));
        optionTopSellers.Add("take", toggleSvr<int>(serverType, 8, 7));
    ...
    protected void rp_plist_ItemDataBound_Buy1free1 (object sender, RepeaterItemEventArgs e)
    {
        rp_plist_ItemDataBound_main(sender, e, optionBuy1free1);
    }
