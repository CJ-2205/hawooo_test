<%@ Page Title="" Language="C#" MasterPageFile="~/user/user.master" AutoEventWireup="true" CodeFile="MidYearSalePreOrder.aspx.cs" Inherits="user_MidYearSalePreOrder" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div class="am-container" style="background-color: #dfdfdf; padding: 15px;">
        <div>
            <img src="<%= ConfigurationManager.AppSettings["imgUrl"] %>ftp/20190625/48h_a_click.png" style="width: 100%" />
        </div>
        <%-- 上方加入商品 --%>
        ...
        <section style="background: black; color: white; padding: 20px 0px 10px 0px;">
            <ul style="line-height: 2;">
                <li>1. Flash sale date: 2nd July 2019 00:00:00 - 3rd July 2019 23:59:59
                </li>
                <li>2. Flash Sale items are at their lowest prices. Promo codes are not applicable to Flash Sale items. Brand activities are NOT applicable on Flash Sale items as well. However, Sitewide Free Gift with minspend RM399 is APPLICABLE on Flash Sale items.
                </li>
                <li>3. 【27th June 2019 00:00:00 - 1st July 2019 23:59:59】 Add your items into wishlist
                </li>
                <li>4. 【2nd July 2019 00:00:00 - 3rd July 2019 23:59:59】 The items in your wishlist will be automatically added into your shopping cart, check out as soon as possible!
                </li>
    ...
    <script>
        ...
        function DoDel(ItemID) {
            ...
            if (memberID !== '') {
                ...
                $.ajax({
                    type: "post",
                    url: "<%= Resources.PreOrderRes.Url.ToString() %>/DoDel",
                    data: val,
                ...
                doLogin("<%= Resources.PreOrderRes.Url.ToString() %>.aspx");
            }
        }
        ...
        function AddEvent(iid, pid, qty) {
            ...
            if (iid !== "") {
                ...
                $.ajax({
                    type: "post",
                    url: "<%= Resources.PreOrderRes.Url.ToString() %>/DoAdd",
                    data: val,
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: false,
        ...
        function BookingEvent(selectProduct) {
            ...
            if (memberID !== '') {
                ...
                doLogin("<%= Resources.PreOrderRes.Url.ToString() %>");
            }
        }
        ...
        function AddOneItem(itemID) {
            ...
            $.ajax({
                type: "post",
                url: "<%= Resources.PreOrderRes.Url.ToString() %>/GetItem",
                data: val,
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                async: false,
        ...
        function GetList() {
            ...
            $.ajax({
                type: "post",
                url: "<%= Resources.PreOrderRes.Url.ToString() %>/GetAddList",
                data: LG,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
        ...
        function SetInfo() {
            ...
            if ('<%= (Master as user_user).LgType %>' === "en") {
                msg = "#QTY# items have been purchased, the original price is RM #PRICE# , and the offer price is RM #SPRICE# .Attention! The items are limited, wishlist will be added into your cart at 02/07 00:00 and you need to  checkout before 03/07 23:59.";

            } else {
                msg = "已选购 #QTY# 件，原价RM #PRICE#，折扣价RM #SPRICE#，注意! 货量有限，预购商品于02/07 00:00会同步至您的购物车, 03/07 23:59 前请抢先结帐.";
            }
