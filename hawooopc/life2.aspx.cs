...
public partial class user_life2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["desktop"] == null)
            {
                if (ismobile)
                {
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "msg", "location.href='../mobile/life2.aspx'", true);
                }
            }
    ...
    private void bindDT()
    {
        ...
        searchProp.JoinTxts.Add("INNER JOIN (SELECT SPD01 AS SPD01,SPD02 AS SPD02 FROM SPRODUCTSD WHERE SPD01 IN ('543','544','545','546') ) AS DT ON  WP01=DT.SPD02  ");
...
if (dt.Select("SPD01='543' ").Length > 0)
        {
            rp0.DataSource = dt.Select("SPD01='543'").CopyToDataTable();
            rp0.DataBind();
        }
        if (dt.Select("SPD01='543' ").Length > 0)
        {
            rp1.DataSource = dt.Select("SPD01='543'").CopyToDataTable();
            rp1.DataBind();
        }
        if (dt.Select("SPD01='544' ").Length > 0)
        {
            rp2.DataSource = dt.Select("SPD01='544'").CopyToDataTable();
            rp2.DataBind();
        }
        if (dt.Select("SPD01='545' ").Length > 0)
        {
            rp3.DataSource = dt.Select("SPD01='545'").CopyToDataTable();
            rp3.DataBind();
        }
