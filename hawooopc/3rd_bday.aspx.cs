...
public partial class user_3rd_bday : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["desktop"] == null)
            {
                if (ismobile)
                {
                    ScriptManager.RegisterClientScriptBlock(Page, typeof(Page), "msg", "location.href='../mobile/3rd_bday.aspx'", true);
                }
            }
    ...
    private void bindDT()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            DataRow[] dr1 = dt.Select("SPD01='529' AND WP31<='" + strDate + "' AND WP32>'" + strDate + "'");
            ...
            if (dr1.Length > 0)
            {
                ...
                rp1.DataSource = dt1;
                rp1.DataBind();
                ScriptManager.RegisterStartupScript(Page, typeof(Page), "setTime", "timeEvent('" + Convert.ToDateTime(dr1[0]["WP31"]).ToString("yyyy-MM-dd HH:mm:ss") + "');", true);
            }

            if (dt.Select("SPD01='528'").Length > 0)
            {
            ...
            {
                rp3.DataSource = dt.Select("SPD01='540'").CopyToDataTable();
                rp3.DataBind();
            }
            if (dt.Select("SPD01='541'").Length > 0)
            {
                rp4.DataSource = dt.Select("SPD01='541'").CopyToDataTable();
                rp4.DataBind();
            }
            if (dt.Select("SPD01='542'").Length > 0)
            {
                rp5.DataSource = dt.Select("SPD01='542'").CopyToDataTable();
                rp5.DataBind();
            }
            if (dt.Select("SPD01='543'").Length > 0)
            {
                rp6.DataSource = dt.Select("SPD01='543'").CopyToDataTable();
                rp6.DataBind();
            }
            if (dt.Select("SPD01='544'").Length > 0)
            {
                rp7.DataSource = dt.Select("SPD01='544'").CopyToDataTable();
                rp7.DataBind();
            }
            if (dt.Select("SPD01='545'").Length > 0)
            {
                rp8.DataSource = dt.Select("SPD01='545'").CopyToDataTable();
                rp8.DataBind();
            }
            if (dt.Select("SPD01='546'").Length > 0)
            {
                rp9.DataSource = dt.Select("SPD01='546'").CopyToDataTable();
                rp9.DataBind();
            }
            if (dt.Select("SPD01='547'").Length > 0)
            {
                rp10.DataSource = dt.Select("SPD01='547'").CopyToDataTable();
                rp10.DataBind();
            }
            if (dt.Select("SPD01='548'").Length > 0)
            {
                rp11.DataSource = dt.Select("SPD01='548'").CopyToDataTable();
                rp11.DataBind();
            }
            if (dt.Select("SPD01='549'").Length > 0)
            {
                rp12.DataSource = dt.Select("SPD01='549'").CopyToDataTable();
                rp12.DataBind();
            }
            if (dt.Select("SPD01='550'").Length > 0)
            {
                rp13.DataSource = dt.Select("SPD01='550'").CopyToDataTable();
                rp13.DataBind();
            }
            if (dt.Select("SPD01='551'").Length > 0)
            {
                rp14.DataSource = dt.Select("SPD01='551'").CopyToDataTable();
                rp14.DataBind();
            }
            ...
            //    {
            //        var addRow = from r in bindLikeDT.AsEnumerable()
            //                     where r.Field<Int64>("WP01").Equals(Convert.ToInt64(dr["WP01"].ToString()))
            //                     select r;
            //        if (addRow != null)
            //            bindLikeDT.Rows.Remove(addRow.FirstOrDefault());
            //    }
            //}
            if (bindLikeDT.Rows.Count > 0)
            {
                Random rand = new Random();
                bindLikeDT = bindLikeDT.AsEnumerable().OrderBy(r => rand.Next()).Skip(1).Take(40).CopyToDataTable();

                rp15.DataSource = bindLikeDT;
                rp15.DataBind();
            }
            //dtLike = dtLike.Rows.Cast<DataRow>().Skip(1).Take(40).CopyToDataTable();
