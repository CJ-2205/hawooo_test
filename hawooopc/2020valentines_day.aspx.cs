...
public partial class user_static_2020valentines_day : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (ismobile)
                Response.Redirect("../mobile/2020valentines_day.aspx" + Request.Url.Query);


            //DataTable dt = BindData(862);
    ...
    //   FROM wp
    //   WHERE B01 IN(208, 72, 102, 115, 235, 345, 283, 450)
    //     AND WP07 = 1
    //     AND GETDATE()
    //     BETWEEN WP09
    //     AND WP10) AS DT
    ...
    private void BindBrandData()
    {
        ...
        DateTime ctime = Convert.ToDateTime("2020-02-14 00:00:00");

        SqlCommand cmd = new SqlCommand();
        StringBuilder sb = new StringBuilder();
        sb.Append(@"SELECT WP.WP02,WP.WP24,WPT02 AS WP30,WPT07,WP.WP01,WP.WP08_1,WP.WP27,Price AS WPA06,OPrice AS WPA10,WP39,WP23,B01
FROM WP WITH(NOLOCK)
INNER JOIN ProductPriceView WITH(NOLOCK)
 ON PID = WP01
LEFT JOIN WPTAG WITH(NOLOCK) ON WPT01 = WP30
...
WHERE WP.WP05 = 1
 ...
 FROM
   ...
   WHERE B01 IN(");
        if (stime < ctime)
        {
            sb.Append("208, 72, 102, 115, 235, 345, 283, 450");
            sb.Append(@")
     AND WP07 = 1
     AND GETDATE()
     BETWEEN WP09
...
END)");
        ...
        {
            sb.Append("307, 72, 115, 102, 235, 345, 283, 450");
            sb.Append(@")
     AND WP07 = 1
     AND GETDATE()
     BETWEEN WP09
...
END)");
    ...
    private void BindTop4ClassData()
    {
        ...
        if (dt.Select("CNAME='內衣'").Length > 0)
        {
            ...
            rp7.DataSource = dt.Select("CNAME='內衣'").Take(4).CopyToDataTable();
            rp7.DataBind();
        }
        ...
        if (dt.Select("CNAME='美食'").Length > 0)
        {
            ...
            rp8.DataSource = dt.Select("CNAME='美食'").Take(4).CopyToDataTable();
            rp8.DataBind();
        }
        ...
        if (dt.Select("CNAME='母嬰'").Length > 0)
        {
            ...
            rp9.DataSource = dt.Select("CNAME='母嬰'").Take(4).CopyToDataTable();
            rp9.DataBind();
        }
    ...
    public DataTable GetGoods(LangType lg, string et = "")
    {
        ...
        sb.Append("INNER JOIN EVENT0204 AS T ON T.PID=WP01 ");
        sb.Append("WHERE NOT EXISTS");
        sb.Append("(SELECT SPD02 FROM SPRODUCTSD WHERE SPD01 IN (862,863) AND WP01=SPD02) ");

        if (et == "top4")
        {
            sb.Append("AND VRANK<7 ");
        }
        if (et == "top15")
        {
